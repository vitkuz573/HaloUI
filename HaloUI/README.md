...
Icons are rendered through `HaloIcon` and resolved via `IHaloIconResolver`. The default resolver is passthrough ligature mode. Hosts can either plug a custom manifest via `AddHaloUIIconPack(...)` or use the embedded Material pack extension from `HaloUI.IconPacks.Material` (`AddHaloUIMaterialIconPack(...)`).
