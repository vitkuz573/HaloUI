...
Icons are rendered through `HaloIcon` and resolved via `IHaloIconResolver`. The default resolver is passthrough ligature mode; hosts can replace it with `AddHaloUIIconPack(...)` and a JSON manifest generated by `tools/HaloUI.IconCatalog.Tool`.
