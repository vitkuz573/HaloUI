...
static async Task<int> GenerateMaterialIconsAsync(Dictionary<string, string?> options)
{
    ...
    var csharpOutput = options.GetValueOrDefault("--csharp-output")
        ?? Path.Combine(output, "HaloMaterialIcons.g.cs");
    ...
    var allIconNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    ...
    if (allIconNames.Count > 0)
    {
        var csharpSource = BuildMaterialIconsSource(allIconNames);
        ...
        Console.WriteLine($"[generated] {csharpOutput} ({allIconNames.Count} icons)");
    }
...
static string BuildMaterialIconsSource(IEnumerable<string> iconNames)
{
    ...
    builder.AppendLine();
    builder.AppendLine("namespace HaloUI.Iconography;");
    builder.AppendLine();
    builder.AppendLine("/// <summary>");
    builder.AppendLine("/// Strongly-typed Material icon tokens generated from the upstream codepoint catalog.");
    builder.AppendLine("/// </summary>");
    builder.AppendLine("public static class HaloMaterialIcons");
    builder.AppendLine("{");

    foreach (var (name, identifier) in entries)
    {
        var emittedIdentifier = EscapeIdentifier(identifier);
        builder.Append("    public static readonly HaloIconToken ").Append(emittedIdentifier)
            .Append(" = HaloIconToken.Create(\"").Append(EscapeForCSharpString(name)).AppendLine("\");");
    }

    builder.AppendLine();
    builder.AppendLine("    private static readonly IReadOnlyDictionary<string, HaloIconToken> Lookup =");
    builder.AppendLine("        new Dictionary<string, HaloIconToken>(StringComparer.OrdinalIgnoreCase)");
    builder.AppendLine("        {");
    ...
    builder.AppendLine();
    builder.AppendLine("    /// <summary>");
    builder.AppendLine("    /// Tries to resolve a material icon token by its canonical string name.");
    builder.AppendLine("    /// </summary>");
    builder.AppendLine("    public static bool TryGet(string iconName, out HaloIconToken iconToken)");
    builder.AppendLine("    {");
    builder.AppendLine("        if (string.IsNullOrWhiteSpace(iconName))");
    builder.AppendLine("        {");
    builder.AppendLine("            iconToken = default;");
    builder.AppendLine("            return false;");
    builder.AppendLine("        }");
    builder.AppendLine();
    builder.AppendLine("        return Lookup.TryGetValue(iconName.Trim(), out iconToken);");
    builder.AppendLine("    }");
    builder.AppendLine("}");
...
static bool IsCSharpKeyword(string identifier)
{
    return identifier switch
    {
        "Abstract" or "As" or "Base" or "Bool" or "Break" or "Byte" or "Case" or "Catch" or "Char" or "Checked" or
            "Class" or "Const" or "Continue" or "Decimal" or "Default" or "Delegate" or "Do" or "Double" or
            "Else" or "Enum" or "Event" or "Explicit" or "Extern" or "False" or "Finally" or "Fixed" or "Float" or
            "For" or "Foreach" or "Goto" or "If" or "Implicit" or "In" or "Int" or "Interface" or "Internal" or
            "Is" or "Lock" or "Long" or "Namespace" or "New" or "Null" or "Object" or "Operator" or "Out" or
            "Override" or "Params" or "Private" or "Protected" or "Public" or "Readonly" or "Ref" or "Return" or
            "Sbyte" or "Sealed" or "Short" or "Sizeof" or "Stackalloc" or "Static" or "String" or "Struct" or
            "Switch" or "This" or "Throw" or "True" or "Try" or "Typeof" or "Uint" or "Ulong" or "Unchecked" or
            "Unsafe" or "Ushort" or "Using" or "Virtual" or "Void" or "Volatile" or "While" => true,
        _ => false
    };
}
...
static void PrintUsage()
{
    Console.WriteLine(
        ...
        material-icons options:
          ...
          --csharp-output <file>     Output .cs file for strongly-typed HaloMaterialIcons catalog
